const SIZE = 9;
const DIFFICULTIES = { Easy: { clues: 43, multiplier: 1 }, Medium: { clues: 35, multiplier: 1.5 }, Hard: { clues: 29, multiplier: 2 }, 'Ultra Hard': { clues: 23, multiplier: 3 } };
...
function clearPeerNotes(row, col, number) {
  for (let index = 0; index < SIZE; index++) {
    state.notes[row][index].delete(number);
    state.notes[index][col].delete(number);
  }
  const boxRow = Math.floor(row / 3) * 3, boxCol = Math.floor(col / 3) * 3;
  for (let rowOffset = 0; rowOffset < 3; rowOffset++) {
    for (let colOffset = 0; colOffset < 3; colOffset++) {
      state.notes[boxRow + rowOffset][boxCol + colOffset].delete(number);
    }
  }
}
function setCellNumber(number) {
  ...
  else { state.values[row][col] = number; state.notes[row][col].clear(); clearPeerNotes(row, col, number); state.errors.delete(state.selected); }
