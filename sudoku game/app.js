...
async function submitScore(){if(state.status!=='solved'||state.submitted||busy)return;const input=document.querySelector('[data-name]');if(!input.value.trim()){input.focus();return;}busy=true;try{applyGame(await api('games/'+state.gameId+'/score',{name:input.value.trim()}));renderGame();setTimeout(()=>{location.hash='high-scores';},1500);}catch(e){setMessage(e.message,'error-message');}finally{busy=false;}}
