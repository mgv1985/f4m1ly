...
function bricks(count, interactive = false) {
  ...
  const images = Array.from({ length: count }, () => '<img src="brick.png" alt="" draggable="false" />').join('');
...
function showIntro() {
  game.innerHTML = `<section class="screen intro-screen"><div><p class="eyebrow">Μαθαίνω παίζοντας</p><h1>Μονάδες, δεκάδες και εκατοντάδες.</h1><p class="screen-copy">Μαθαίνω τις μονάδες, δεκάδες και εκατοντάδες.</p><div class="intro-actions"><button class="primary-action" id="beginLesson" type="button">Ξεκινάμε</button></div></div><div class="intro-art"><img src="castle.png" alt="Κόκκινο κάστρο φτιαγμένο από τουβλάκια" /></div></section>`;
  document.querySelector('#beginLesson').addEventListener('click', () => showLesson(1));
}

function showLesson(step) {
  const left = step === 1 ? 1 : 10;
  const right = step === 1 ? 10 : 100;
  game.innerHTML = `<section class="screen lesson-screen"><div class="lesson-title"><p class="eyebrow">Παρατήρησε τα τουβλάκια</p><h1>${concepts[left].option} και ${concepts[right].option}</h1></div><div class="compare-grid"><div class="brick-panel">${bricks(left, true)}</div><div class="brick-panel">${bricks(right, true)}</div></div><button class="next-action" id="lessonNext" type="button">Επόμενο →</button></section>`;
  bindSpokenTargets();
  document.querySelector('#lessonNext').addEventListener('click', () => step === 1 ? showLesson(2) : startQuiz());
}
...
function showQuestion() {
  ...
  game.innerHTML = `<section class="screen quiz-screen"><div class="quiz-title"><p class="quiz-progress">${questionIndex + 1} / 3</p><h1>Τι βλέπεις;</h1></div><div class="quiz-visual">${bricks(correct)}</div><div class="answer-options">${[1, 10, 100].map(value => `<button type="button" data-answer="${value}" data-word="${concepts[value].word}">${concepts[value].option}</button>`).join('')}</div></section>`;
...
function chooseAnswer(button, correct) {
  ...
  setTimeout(() => {
    questionIndex += 1;
    if (questionIndex < 3) showQuestion(); else showResults();
  }, 650);
}

function resultText() {
  if (score === 3) return { title: 'Μπράβο, πέτυχες και τα τρία!', copy: '' };
  if (score === 2) return { title: '2/3, τα πήγες καλά.', copy: 'Ξαναδοκίμασε για να τα πας καλύτερα.' };
  if (score === 1) return { title: '1/3 Καλή προσπάθεια.', copy: 'Ξαναδοκίμασε για να τα πας καλύτερα.' };
  return { title: '0/3 Καλή προσπάθεια.', copy: 'Προσπάθησε ξανά για να τα πας καλύτερα.' };
}
